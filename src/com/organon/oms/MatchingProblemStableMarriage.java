 ...
 * matching problem. In mathematics, economics, and computer science, the stable
 * marriage problem (SMP) is the problem of finding a stable matching between
 * two sets of elements given a set of preferences for each element. A matching
 * is a mapping from the elements of one set to the elements of the other set. A
 * matching is stable whenever it is not the case that both: 1. some given
 * element A of the first matched set prefers some given element B of the second
 * matched set over the element to which A is already matched, and 2. B also
 * prefers A over the element to which B is already matched
 * 
 * In other words, a matching is stable when there does not exist any
 * alternative pairing (A, B) in which both A and B are individually better off
 * than they would be with the element to which they are currently matched.
 * 
 * The stable marriage problem is commonly stated as: Given n men and n women,
 * where each person has ranked all members of the opposite gender with a unique
		...
		for (Map.Entry<String, String> couple : matches.entrySet()) {
			...
			for (String guy : sheLikesBetter) {
				...
				if (thisGuyPrefers.indexOf(guysFinace) > thisGuyPrefers.indexOf(couple.getKey())) {
					System.out.printf(
							"%s likes %s better than %s and %s" + " likes %s better than their current partner\n",
