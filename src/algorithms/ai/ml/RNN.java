	...
	public static void main(String[] args) {
		...
		/*
		 	...
		 	1. Define the neural network structure 
		 		1.1. n_x: the size of the input layer
		 		1.2. n_h: the size of the hidden layer (set this to 4)
		 		1.3. n_y: the size of the output layer
		...
		/**
		 * Interpretation: Recursive Neural Network
...
You will also learn later about regularization, which lets you use very large models (such as n_h = 50) without much overfitting.
		...
		/**
		 * Base of Artificial Intelligence (Neural Network and Machine Learning Process with Recursive Neural Network - RNN)
		 * Convulotional Neural Network don't apply here
		 * 
		 * The Loop
		  ...
		  Instructions:
			...
			4. The steps you have to implement are:
				4.1. Retrieve each parameter from the dictionary "parameters" (which is the output of initialize_parameters()) by using parameters[".."].
				4.2. Implement Forward Propagation. Compute  Z[1],A[1],Z[2] and  A[2]
		...
		for (int i = 0; i < 100000; i++) {
			...
			//Now that you have computed  A[2] (in the Python variable "A2"), which contains  a[2](i)
...
//		np.print("Standard deviation = " + sd);
	...
	private static void verifyTrainingIO(double[][] X, double[][] Y) {
		...
		if(teFromX!=teFromY) {
			System.out.println("Input have "+teFromX+" training examples and Output have "+teFromY+" training examples!");
	...
	public double[][] accuracy(double[][] predictions, double[][] Y) {
		double[][] r =  np.add(np.dot(Y, np.T(predictions)), np.dot(np.subtract(1.0, Y),np.subtract(1.0,np.T(predictions))));
